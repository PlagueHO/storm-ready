...
1. From the repo root, verify GitHub CLI auth and install the Agentic Workflows extension:
   ```bash
   gh --version
   gh auth status
   gh extension install github/gh-aw
   ```
2. Add the sample workflow (interactive wizard) and preselect the Copilot engine to keep it fast live:
   ```bash
   gh aw add-wizard githubnext/agentics/daily-repo-status --engine copilot
   ```
3. If prompted for Copilot auth, set the required repo secret (token created beforehand):
   ```bash
   gh secret set COPILOT_GITHUB_TOKEN < .secrets/copilot-token.txt
   ```
4. Trigger a run immediately and watch progress:
   ```bash
   gh aw run daily-repo-status
   gh aw status
   gh run watch
   ```
5. Open `.github/workflows/daily-repo-status.md`, tweak one section (for example "What to include" to focus on stale issues and flaky tests), then recompile and rerun:
   ```bash
   gh aw compile
   gh aw run daily-repo-status
   ```
6. Show the generated report issue on GitHub.com and explain how recurring maintenance (triage, backlog health, release hygiene) now runs on a schedule.

**Handoff to RIC:** "This all gets better when the AI knows _our_ organization."
